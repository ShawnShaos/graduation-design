(function(root, factory) {
	...
	V = Vectorizer = function() {
		...
		d.prototype.getTransformToElement = function(a) {
			...
			if(d.isUndefined(a)) {
				for(var c = this.node.attributes, e = {}, f = 0; f < c.length; f++) e[c[f].nodeName] = c[f].nodeValue;
				return e
			}
...
//	})(typeof window !== _0x53da[0] ? window : null);
	...
	joint.mvc.View = Backbone.View.extend({
		...
		function() {
			...
			_prepareCell: function(a) {
				var b;
				if(a instanceof Backbone.Model ? (b = a.attributes, a.graph = this) : b = a, !_.isString(b.type)) throw new TypeError("dia.Graph: cell type must be a string.");
				return a
			},
			maxZIndex: function() {
			...
			addCell: function(a, b) {
				return _.isArray(a) ? this.addCells(a, b) : (a instanceof Backbone.Model ? a.has("z") || a.set("z", this.maxZIndex() + 1) : _.isUndefined(a.z) && (a.z = this.maxZIndex() + 1), this.get("cells").add(this._prepareCell(a), b || {}), this)
			},
			...
			update: function(a, b) {
				...
				_.each(b || c, function(a, b) {
					...
					0 !== c.length && (g[b] = c, this.updateAttr(c, a), _.isUndefined(a["ref-x"]) && _.isUndefined(a["ref-y"]) && _.isUndefined(a["ref-dx"]) && _.isUndefined(a["ref-dy"]) && _.isUndefined(a["x-alignment"]) && _.isUndefined(a["y-alignment"]) && _.isUndefined(a["ref-width"]) && _.isUndefined(a["ref-height"]) || _.each(c, function(a, b, c) {
						var d = $(a);
		...
		joint.shapes.basic.Ellipse = joint.shapes.basic.Generic.extend({
			...
			initialize: function() {
				joint.dia.ElementView.prototype.initialize.apply(this, arguments), this.noSVGForeignObjectElement = !joint.env.test("svgforeignobject"), joint.env.test("svgforeignobject") || this.listenTo(this.model, "change:content", function(a) {
	...
	graphlib = graphlib || "undefined" != typeof window && window.graphlib, dagre = dagre || "undefined" != typeof window && window.dagre, joint.layout.DirectedGraph = {
		layout: function(a, b) {
			var c;
			c = a instanceof joint.dia.Graph ? a : (new joint.dia.Graph).resetCells(a), a = null, b = _.defaults(b || {}, {
				resizeClusters: !0,
				clusterPadding: 10
			});
			var d = c.toGraphLib({
		...
		revertCommand: function(a, b) {
			...
			for(var d = c.length - 1; d >= 0; d--) {
				var e = c[d],
					...
					}, b, _.pick(e.options, this.get("revertOptionsList")));
				switch(e.action) {
					case "add":
						f.remove(g);
						break;
					case "remove":
						this.graph.addCell(e.data.attributes, g);
						break;
					default:
						var h = e.action.substr(this.PREFIX_LENGTH);
						f.set(h, e.data.previous[h], g)
				}
			}
			this.listen()
		},
		applyCommand: function(a, b) {
			...
			for(var d = 0; d < c.length; d++) {
				var e = c[d],
					...
					g = _.extend({
						commandManager: this.id || this.cid
					}, b, _.pick(e.options, this.get("applyOptionsList")));
				switch(e.action) {
					case "add":
						this.graph.addCell(e.data.attributes, g);
						break;
	...
	joint.ui.Selection = joint.mvc.View.extend({
		options: {
			...
			boxContent: function(a) {
				return joint.util.template("<%= length %> 个设备被选中.")({
...
//					var d = a.util.template("x: <%= x %>, y: <%= y %>, width: <%= width %>, height: <%= height %>, angle: <%= angle %>"),
		...
		d.prototype.startLinking = function(a, c, d) {
			...
			onHandlePointerDown: function(b) {
				if(this._action = $(b.target).closest(".handle").attr("data-action"), this._action) {
					...
					this._localX = c.x, this._localY = c.y, this.triggerAction(this._action, "pointerdown", b, c.x, c.y)
				}
			},
			...
			pointermove: function(b) {
				if(this._action) {
					...
					var c = this.options.paper.snapToGrid({
						...
						d = c.x - this._localX,
						e = c.y - this._localY;
					this._localX = c.x, this._localY = c.y, this.triggerAction(this._action, "pointermove", b, c.x, c.y, d, e)
				}
			},
			pointerup: function(a) {
				if(this._action) {
					...
					this.triggerAction(this._action, "pointerup", a, b.x, b.y), this._action = null
				}
			},
			onRemove: function() {
	...
	! function(a, b) {
		...
		a.ui.Stencil = a.mvc.View.extend({
			...
			onDragEnd: function(b) {
				...
				if(c) {
					...
					var i = this.options.dragEndClone(c),
						j = this.drop(b, i);
					j ? this.onDropEnd() : this.onDropInvalid(b, i), this.options.paper.options.embeddingMode && d && d.finalizeEmbedding({
						model: i,
						paper: this.options.paper
			...
			onDropEnd: function() {
				this.$el.append(this._paperDrag.$el), this.$el.removeClass("dragging"), this._paperDrag.$el.removeClass("dragging"), this._clone.remove(), this._clone = null, this._cloneView = null, this._cloneSnapOffset = null
			},
			onDropInvalid: function(c, d) {
				if(this._clone) {
					c = a.util.normalizeEvent(c), d = d || this.options.dragEndClone(this._clone), this.trigger("drop:invalid", c, d);
					var e = this.options.dropAnimation;
					if(e) {
						var f = b.isObject(e) ? e.duration : 150,
							g = b.isObject(e) ? e.easing : "swing";
						this._paperDrag.$el.animate(this._paperDragInitialOffset, f, g, this.onDropEnd)
					} else this.onDropEnd()
				}
			},
	...
	joint.ui.SelectBox = joint.mvc.View.extend({
		...
		init: function() {
			_.bindAll(this, "onOutsideClick", "onOptionSelect"), $(document).on("click.selectBox", this.onOutsideClick), this.$el.data("view", this), _.isUndefined(this.options.selected) ? this.selection = _.findWhere(this.options.options, {
				selected: !0
	...
	joint.ui.Navigator = joint.mvc.View.extend({
		...
		render: function() {
			if(this.targetPaper.$el.appendTo(this.el), this.sourcePaper.model.get("cells").each(this.targetPaper.renderView, this.targetPaper), this.$currentView = $("<div>").addClass("current-view"), this.options.zoom) {
				var a = $("<div>").addClass("current-view-control");
				this.$currentView.append(a)
			}
	...
	joint.layout = joint.layout || {}, joint.layout.GridLayout = {
		layout: function(a, b) {
			var c;
			c = a instanceof joint.dia.Graph ? a : (new joint.dia.Graph).resetCells(a), a = null, b = b || {};
			var d = c.getElements(),
				e = b.columns || 1,
				f = b.dx || 0,
				g = b.dy || 0,
	...
	! function(a, b, c, d) {
		...
		var g = e.extend({
			...
			k = {
				getTopDY: function() {
					var a = this.layoutAreas[0];
					return a ? a.prevSiblingGap : 0
				}
			},
			l = {
				getTopDY: function() {
